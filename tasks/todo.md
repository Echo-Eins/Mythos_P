...
# Compile Grad-Accum CUDA Graph Marker Fix

- [x] Fix `torch.compile(..., reduce-overhead)` crash during gradient accumulation by marking CUDA Graph step boundaries before compiled forwards.
- [x] Make preflight simulate multiple micro-batches when `--grad-accum > 1` instead of only a single forward/backward.
- [x] Verify syntax locally and rerun Spark compile preflight with the real train batch/grad-accum shape before full training.

## Compile Grad-Accum Review

- Added `mark_compiled_model_step()` and call it before every compiled model forward in train, eval, and preflight.
