...
- On the current Spark torch/Inductor stack, `torch.compile(mode="reduce-overhead")` is not reliable with `grad_accum > 1` for this model even with CUDA Graph step markers. Fail fast and prefer no compile for serious runs until a separate compile mode is proven by a matching multi-microbatch preflight.
